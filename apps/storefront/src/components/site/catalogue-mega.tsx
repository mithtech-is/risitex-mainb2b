...
function SubLink({
  ...
  return (
    ...
    >
      ...
      <ChevronRight
        className="h-3.5 w-3.5 shrink-0 -translate-x-1 text-text-muted opacity-0 transition-all duration-fast group-hover/sub:translate-x-0 group-hover/sub:opacity-100 group-hover/sub:text-text-primary"
        aria-hidden
      />
    </Link>
  );
...
/**
 * Premium Catalogue mega-menu. Hovering (or focusing) "Catalogue" reveals a
 * panel of category sections — each root category is a column with its
 * subcategories, plus a featured CTA card. Data-driven from Medusa so new
 * admin categories appear with zero code change. Desktop only; the mobile
 * sheet keeps its simple list.
 */
export function CatalogueMega() {
  ...
  return (
    ...
    >
      ...
      >
        ...
        <span
          className={`pointer-events-none absolute -bottom-1.5 left-0 h-px bg-text-primary transition-all duration-base ease-standard ${
            active ? "w-full" : open ? "w-full" : "w-0 group-hover:w-full"
          }`}
          aria-hidden
        />
      </Link>
      ...
      {/* Panel */}
      <div
        className={`absolute left-0 top-full z-popover pt-3 transition-all duration-base ease-standard ${
          open
      ...
      >
        <div className="flex overflow-hidden rounded-2xl border border-border-subtle bg-surface-raised shadow-popover">
          {/* Category sections */}
          <div className="flex gap-8 p-6">
            {roots.length === 0 ? (
              ...
              roots.map((root) => (
                <div key={root.id} className="min-w-[184px]">
                  <Link
                    href={catHref(root.handle)}
                    onClick={closeNow}
                    className="mb-2 block px-3 text-caption font-semibold uppercase tracking-[0.14em] text-text-muted transition-colors duration-fast hover:text-text-primary"
                  >
                    {root.name}
                  </Link>
          ...
          <Link
            href="/wholesale/catalogue"
            onClick={closeNow}
            className="group/cta relative flex w-56 shrink-0 flex-col justify-end overflow-hidden bg-action-primary-bg p-6 text-action-primary-text transition-colors duration-base hover:bg-action-primary-bg-hover"
          >
            <span
              aria-hidden
              className="absolute -right-6 -top-6 h-24 w-24 rounded-full bg-action-primary-text/10 blur-2xl transition-transform duration-base group-hover/cta:scale-125"
            />
            <span className="text-caption font-semibold uppercase tracking-[0.14em] text-action-primary-text/60">
              Wholesale
            </span>
            <span className="mt-1 text-heading-sm font-semibold leading-tight">
              Browse the full range
            </span>
            <span className="mt-3 inline-flex items-center gap-1.5 text-body-sm font-medium">
              Shop all products
