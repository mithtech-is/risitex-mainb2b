...
/**
 * Paginated viewer for outbound SMS attempts. Body column is sensitive
 * (contains plaintext OTPs for OTP sends) — render it inside a
 * collapsed "show body" panel rather than directly in the table cell.
 */
type LogRow = {
    id: string
    to_phone: string
...
export default function SmsLogsTab() {
    ...
    const load = async (nextOffset = offset) => {
        ...
        try {
            ...
            const r = await fetch(
                `/admin/communication/sms-logs?${params.toString()}`,
                { credentials: "include" },
            )
